...
def get_plot_dataframe(data_folders, runs, reqd_keys):
    ...
    name = f'er{exp_edges}_d{d}_D{D}_proj{proj}_{interv_type}'
    return plot_df, name

def fetch_and_plot_num_intervs_ablation(num_nodes, num_intervs, basepath, runs, reqd_keys, proj='linear', interv_type='multi', box_widths=0.8, fontsize=18):
    zfilled_nodes = str(num_nodes).zfill(3)
    data_folders = []
    for num_interv in num_intervs:
        datafolder = f'er1-ws_datagen_fix_noise_interv_noise-{proj}proj-d{zfilled_nodes}-D0100-{interv_type}-n_pairs{num_interv}-sets{int(num_interv/100)}-gaussianinterv'
        data_folders.append(datafolder)

    plot_df, name = get_plot_dataframe(data_folders, runs, reqd_keys)
    utils.plot_num_interventions_ablation(plot_df, basepath, name, reqd_keys, fontsize, num_intervs, box_widths=box_widths, title=None)