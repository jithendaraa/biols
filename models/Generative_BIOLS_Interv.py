...
class GenerativeBIOLS_Interv(hk.Module):
    def __init__(
        self, 
        d, 
        posterior_samples, 
    ...
    def gumbel_softmax(self, rng_key, logits, tau=1., axis=-1):
        ...
        return hard_samples - lax.stop_gradient(soft_samples) + soft_samples, soft_samples[:, -1]

    def __call__(self, rng_key, interv_values, LΣ_params, interv_logit_params, hard):
        """
        ...
        # Learn intervention targets
        extended_interv_target, pred_is_observ_probas = self.gumbel_softmax(rng_key, interv_logit_params) # (n, d+1)
        pred_is_observ = extended_interv_target[:, -1]
        pred_interv_targets = extended_interv_target[:, :-1]
        pred_interv_targets = self.interv_targets
        ...
        if self.learn_L:    L_samples = vmap(self.lower, in_axes=(0))(full_l_batch[:,  :self.l_dim]) # (posterior_samples, d, d)
        else:               L_samples = jnp.tile(self.L[None, :, :], (self.posterior_samples, 1, 1))
