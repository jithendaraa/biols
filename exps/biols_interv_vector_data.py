...
z_gt, interv_nodes, x_gt, P, gt_interv_targets, interv_values = datagen.get_data(
                                                                    rng_key,
                                                                    opt, 
                                                                    opt.n_interv_sets, 
                                                                    sd, 
...
(forward, model_params, LΣ_params, model_opt_params, 
    LΣ_opt_params, opt_model, opt_LΣ  ) = init_interv_model(
                                                        key, 
                                                        False, 
                                                        rng_key, 
                                                        opt, 
...
with tqdm(range(10)) as pbar:
    for i in pbar:
        ...
        (   loss, 
            ...
            X_recons, 
            pred_W_means,
            interv_target_samples    ) = gradient_step(model_params, LΣ_params, rng_key, z_gt, x_gt, interv_values)
        
        interv_target_samples = interv_target_samples.astype(bool)
        correct_targets = jnp.sum((gt_interv_targets == interv_target_samples).astype(int))
