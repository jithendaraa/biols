...
def evaluate(rng_key, forward, X, loss, log_dict, opt, params, interv_labels, interv_values, 
    z_samples, z_gt, gt_W, gt_sigmas, perm, edge_threshold=0.3):
    """
    """
    eval_mean_args = (
        rng_key,
    ...
    eval_mean_kwargs = {'edge_threshold': edge_threshold}
...
def eval_mean(rng_key, forward, X, opt, params, interv_labels, interv_values, z_data, 
    gt_W, gt_sigmas, perm, edge_threshold=0.3):
    ...
    forward_kwargs = {'P': perm}

    model_outputs = forward.apply(*forward_args, **forward_kwargs)
    _, _, _, _, full_l_batch, _, _, W_samples, _ = model_outputs
