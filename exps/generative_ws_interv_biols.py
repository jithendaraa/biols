...
from modules.biols_model_init import init_generative_interv_model
...
forward, params, opt_params, optimizers = init_generative_interv_model(
    hk_key, 
    False, 
    rng_key, 
    opt, 
...
model_params, LΣ_params, interv_logit_params = params
model_opt_params, LΣ_opt_params, interv_logit_opt_params = opt_params
opt_model, opt_LΣ, opt_interv_logit = optimizers
...
def calc_neg_elbo(model_params, LΣ_params, interv_logit_params, rng_key, gt_x_data, interv_nodes, interv_values):
    """
    ...
    # obs_classification_loss = utils.bce_loss(gt_obs_targets, pred_is_observ_probas)
    # interv_classification_loss = utils.batched_cross_entropy_loss(concat_gt_interv_targets, extended_interv_targets)
    return neg_elbo


@jit
def gradient_step(model_params, LΣ_params, interv_logit_params, rng_key, gt_z_data, gt_x_data, interv_nodes, interv_values):
    """
    ...
    get_loss_and_grad = value_and_grad(calc_neg_elbo, argnums=(0, 1, 2))
    ...
    model_grads, LΣ_grads, interv_logit_grads = tree_map(lambda x_: x_, grads)
    ...
    if opt.eq_noise_var:    log_noise_std_samples = jnp.tile(log_noise_std_samples, (1, d))
    ...
    true_obs_KL_term_Z = vmapped_kl(p_z_obs_joint_covar, p_z_obs_joint_mu, batch_q_z_obs_joint_covars, batch_q_z_obs_joint_mus, opt)
    ...
    return (loss, rng_key_, model_grads, LΣ_grads, interv_logit_grads, 
            log_dict, W_samples, z_samples, pred_X, pred_W_means, 
            extended_interv_target, pred_is_observ_probas, pred_is_observ)


@jit
def update_params(model_grads, model_opt_params, model_params, 
    LΣ_grads, LΣ_opt_params, LΣ_params,
    interv_logit_grads, interv_logit_opt_params, interv_logit_params):
    """
    ...
    interv_logit_updates, interv_logit_opt_params = opt_interv_logit.update(interv_logit_grads, interv_logit_opt_params, interv_logit_params)
    interv_logit_params = optax.apply_updates(interv_logit_params, interv_logit_updates)
    ...
    return model_params, model_opt_params, interv_logit_params, interv_logit_opt_params, LΣ_params, LΣ_opt_params
...
with tqdm(range(opt.num_steps), miniters=tqdm_update_frequency) as pbar:
    for i in pbar:
        ...
        if jnp.any(jnp.isnan(LΣ_grads)) or jnp.any(jnp.isnan(interv_logit_grads)):
            raise Exception("Got NaNs in L grads or interv logit grads.")
        ...
        if i == 0 or (i+1) % tqdm_update_frequency == 0:
            ...
            obs_classification_loss, obs_classification_accuracy = utils.get_obs_classification_accuracy(pred_is_observ_probas, pred_is_observ, gt_obs_targets)
        ...
        if jnp.any(jnp.isnan(ravel_pytree(LΣ_params)[0])) or jnp.any(jnp.isnan(ravel_pytree(model_params)[0])) or jnp.any(jnp.isnan(ravel_pytree(interv_logit_params)[0])):   
